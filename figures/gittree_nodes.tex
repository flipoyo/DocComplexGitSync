...
\node[rootnode] (root) at (0,0) {
  \textbf{root WorkingRepo}\\NodeType = ROOT
};

% Two direct children of root
\node[parentnode] (parent) at (-3,-2) {
  \textbf{nested WorkingRepo}\\NodeType = PARENT
};
\node[leafnode] (lib) at (3,-2) {
  \textbf{leaf WorkingRepo}\\NodeType = LEAF
};
...
% Annotations
\node[label, right=0.3cm of root] {initialise(.cgs) seeds the root runtime entry};
\node[label, right=0.2cm of parent] {initialise discovers nested repos recursively};
\node[label, right=0.2cm of lib] {READY tree leaf tracked in the saved .gts};
\node[label, right=0.2cm of sub1] {nested\_config = "auto", resolved (no nested .cgs)};
\node[label, right=0.2cm of sub2] {nested\_config = "auto", resolved (no nested .cgs)};
\node[label, below=0.15cm of root] {cross-references are resolved to a DAG by the SCC-based cycle-breaking engine};

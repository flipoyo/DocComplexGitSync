...
\node[tier, fill=blue!12, draw=blue!40] (api) at (0,0) {
  ...
  \texttt{ComplexGitSyncClient} \ $\cdot$ \ \texttt{Orchestre} \ $\cdot$ \ \texttt{cgitsync}\\
  \textit{Single workflow exposure through Python or CLI}
};
...
\node[tier, fill=green!10, draw=green!40] (actions) at (0,-2.3) {
  ...
  \texttt{initialise} \ $\cdot$ \ \texttt{load} \ $\cdot$ \ \texttt{pull} \ $\cdot$
  \ \texttt{clone} \ $\cdot$ \ \texttt{checkout}\\
  \texttt{add} \ $\cdot$ \ \texttt{git(commit)} \ $\cdot$ \ \texttt{git(push)} \ $\cdot$
  \texttt{git(tag)} \ $\cdot$ \ \texttt{freeze}\\
...
\node[tier, fill=orange!10, draw=orange!40] (core) at (0,-4.6) {
  ...
  \texttt{GitTree} \ $\cdot$ \ \texttt{GitRepo} \ $\cdot$ \ \texttt{WorkingGitTree}
  \ $\cdot$ \ \texttt{WorkingRepo}\\
  \textit{Reference identities plus the \texttt{WorkingGitTree} runtime graph}
};

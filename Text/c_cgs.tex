...
\label{subsec:cgs-pinned}

Tree-wide commands --- \texttt{branch}, \texttt{checkout}, \texttt{pull} and
\texttt{pull-force} --- normally drive every repository in the tree onto one
branch. That is the right behaviour when every repository belongs to the
project. It is the wrong behaviour when a repository is \emph{shared}: a
specification repository mounted by several projects, or one whose branch
carries a single project's identity. Creating a project's feature branch
inside a repository that other projects also mount pollutes them all.

Setting \texttt{pinned = true} keeps a repository on its own
\texttt{default\_branch}:

\begin{lstlisting}
repos = [
    "github:flipoyo/ComplexGitSync",
    { repository = "github:flipoyo/.agentSpec", default_branch = "main",
      nested_config = "auto", pinned = true },
    { repository = "github:flipoyo/.localSpec", default_branch = "MyProject",
      fallback_branch = "main", pinned = true },
]
\end{lstlisting}
...
repository back on the commit the snapshot records, pinned or not, so a
release stays reproducible. And a repository declared in a \emph{nested}
\.cgs is pinned by that file, which lives in the repository that owns it ---
...
\texttt{pinned} & boolean & no & \texttt{false} \\

...
\texttt{<CGSHOME>/CGSih1}, referencing the sibling declared elsewhere. No
\texttt{nested\_config} override is needed: discovery's absolute-path dedup
guard already skips re-registering \texttt{CGSih1} once it is known under
its canonical entry, before \texttt{"auto"} ever gets a chance to reopen
its \texttt{.cgs}.
...
\begin{description}
  \item[\texttt{auto}] (default) Loads the sole root-level \texttt{*.cgs} file if
        one exists; finding none is a normal leaf, not an error; multiple
  ...
  \item[a relative path] A path ending in \texttt{.cgs} loads that exact file;
        if the file doesn't exist, this is treated as an error, since an
        explicit path asserts that a specific file must be there.
\end{description}
...
\label{subsec:cgs-private}

A tree holds two kinds of repository. \textbf{Project repos} are the work
itself and follow the project's branch. \textbf{Private repos} are how the
project is run --- pipelines, agent instructions, house rules --- and are
shared with the author's other projects, so they must not follow it:
creating a project's feature branch inside a repository other projects also
mount pollutes them all.

Setting \texttt{private = true} keeps a repository off the project's
branch. Adding \texttt{writable = true} makes it \textbf{private/local} ---
this project's own settings, on a branch named after the project, which
\cgscli{} may write to. Without it the repository is
\textbf{private/distant}: read-only, because it belongs to someone else.

\begin{lstlisting}
repos = [
    "github:flipoyo/ComplexGitSync",
    { repository = "github:flipoyo/.agentSpec", default_branch = "main",
      nested_config = "auto", private = true },
    { repository = "github:flipoyo/.localSpec", default_branch = "MyProject",
      fallback_branch = "main", private = true, writable = true },
]
\end{lstlisting}

With that in place, \texttt{cgitsync branch feature-x} creates
\texttt{feature-x} in the project's own repositories and in neither mount;
\texttt{cgitsync checkout} leaves both on their declared branch; and
\texttt{cgitsync pull} still updates them, on that branch rather than the
tree's.

Two limits are worth knowing. Pinning governs \emph{branch} propagation
only: restoring a frozen release with \texttt{launch-release} puts every
repository back on the commit the snapshot records, private or not, so a
release stays reproducible. And a repository declared in a \emph{nested}
\.cgs is private by that file, which lives in the repository that owns it ---
pinning it means publishing that change there, not in the parent project.
...
\texttt{private} & boolean & no & \texttt{false} \\
...
repos = [
    ...
    { repository = "github:flipoyo/HydrologicalTwinAlphaSeries",
      ...
      fallback_branch = "main" },
    { repository = "github:flipoyo/hydrological_twin",
      relative_path = "external/HydrologicalTwinAlphaSeries/docs/hydrological_twin",
      fallback_branch = "main" },
...
Four repositories on two levels. The third entry's path lies inside the
second's, which is how a \texttt{.cgs} says that one repository holds
another: every path is written from the project root, so nesting shows up
as one path lying inside another, and the innermost holder wins.
\texttt{HydrologicalTwinAlphaSeries} is therefore read back as a
\texttt{PARENT} with \texttt{hydrological\_twin} as its child.

No repository here has a \texttt{.cgs} of its own, but no
\texttt{nested\_config} override is written for that: the default
\texttt{"auto"} already resolves a repository with zero nested
\texttt{*.cgs} matches cleanly.
...
\begin{itemize}
  ...
  \item \textbf{Use \texttt{nested\_config = "disabled"} judiciously:}
        The default \texttt{"auto"} already resolves cleanly whether or not
        a repo has its own \texttt{.cgs} — including cross-references and
        cycle back-references, which the discovery dedup guard already
        prevents from re-registering. Reserve \texttt{"disabled"} for the
        rarer case where a repo does carry a \texttt{.cgs} you explicitly
        want ComplexGitSync to never open.

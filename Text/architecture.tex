...
\input{figures/positioning_matrix}
\end{figure}
...
searches the repository root for exactly one \texttt{*.cgs} — finding zero
resolves the repository as a normal leaf, finding more than one raises
immediately — \texttt{disabled} stops discovery through that reference, and
a relative \texttt{.cgs} path selects an exact file inside the repository,
failing if that named file does not exist. Explicit paths may not escape
the repository root.
...
Each \texttt{repo\_state} record also carries the repository's provider
identity: \texttt{gitprovider}, \texttt{group\_name}, and
\texttt{gitprovider\_url} (mirroring the same fields a \texttt{.cgs} repo
entry carries — \S\ref{chap:user-guide} ``Local Authoring Spec''), plus
\texttt{access\_protocol}. \texttt{gitprovider}/\texttt{group\_name}/
\texttt{gitprovider\_url} are part of the canonical hashed payload:
they say \emph{which repository this is}, so two snapshots pointing at
different hosts are not the same state. \texttt{access\_protocol} is
deliberately excluded from the hash — it records only how the repository
was last reached (ssh or https), not what it is, and a tree cloned entirely
over ssh must hash identically to the same tree cloned over https. A
\texttt{.gts} written before this schema addition has no
\texttt{gitprovider} field at all; every command that would otherwise
derive a clone URL from it (only the missing-repository restore path —
\texttt{push}/\texttt{pull} instead convert the repository's actual
existing remote in place, never rebuilding it from identity) refuses with
an explicit error instead of silently defaulting to GitHub.

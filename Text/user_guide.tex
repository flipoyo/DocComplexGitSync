...
The repository also keeps four checked-in, didactically ordered tutorials
...
\texttt{make}-based build workflow;
\texttt{docs/tutorials/03\_configuration\_discovery\_modes.md} covers the
real-world \texttt{cawaqsviz} project (not synthetic) through two of the
three ways \cgspkg{} can arrive at a working \texttt{.cgs} for a project it
does not already control the source of (hand-authored, and
\texttt{discover}); and
\texttt{docs/tutorials/04\_submodules\_to\_ready.md} covers the
third way (\texttt{import-submodules}, for a project that already tracks
its children as git submodules), then takes any of the three resulting
\texttt{.cgs} files to a working \texttt{READY} tree.

\texttt{examples/cawaqsviz.cgs}, the hand-authored route from
\texttt{docs/tutorials/03\_configuration\_discovery\_modes.md} above,

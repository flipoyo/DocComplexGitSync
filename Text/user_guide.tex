...
there and not at \texttt{Thing}. A repository that contains no
\texttt{*.cgs} of its own is pinned to \texttt{nested\_config = "disabled"},
since leaving it on the default \texttt{auto} would make nested discovery
hunt for a file that does not exist.
...
\texttt{.gitignore} lifecycle sync described there), but inserts a cleanup
...
git submodule to a plain ComplexGitSync nested clone.

Without \texttt{--apply} (the default), the command performs a
\emph{dry run}: it prints each submodule's name, path, URL, and branch, and
the \texttt{.cgs} entry that would be generated, without touching the
repository.
...
The \texttt{--output FILE} option (only effective with \texttt{--apply})
writes a validated \texttt{.cgs} snippet containing the converted submodule
entries to \texttt{FILE}, suitable for integration into the project's main
\texttt{.cgs} file.
...
\texttt{docs/tutorials/03\_configuration\_discovery\_modes.md} covers the
real-world \texttt{cawaqsviz} project (not synthetic) through all three
ways \cgspkg{} can arrive at a working \texttt{.cgs} for a project it does
not already control the source of.

\texttt{examples/cawaqsviz.cgs}, one of that third tutorial's routes,
...
\texttt{docs/CWV\_user\_guide}), and \texttt{nested\_config = "disabled"}
on both children, since neither has a \texttt{.cgs} of its own --- without
it, the default \texttt{auto} nested discovery looks for one and fails.
This file previously shipped with a nonexistent repository identifier, an
invalid \texttt{default\_branch}, and one missing \texttt{nested\_config}
override; it was corrected and given real end-to-end test coverage
...
Phase~1. See \texttt{docs/tutorials/03\_configuration\_discovery\_modes.md}

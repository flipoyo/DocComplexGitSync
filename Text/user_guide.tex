...
\paragraph{A branch this workspace already knows is joined, not recreated.}
When the repository holds a remote-tracking ref for the branch --- because
some earlier \texttt{pull} downloaded it --- \cgscli{} creates the local
branch \emph{from that ref}, with tracking set. You land on your
colleague's commits. Only when no such ref exists does it create a new
branch where you currently stand.
...
in every case and report \texttt{READY}/\texttt{ALIGNED} --- so typing a
colleague's branch name gave you a branch of that name holding commits
that had nothing to do with theirs, and nothing in the output said so.

\texttt{pull} is what puts those refs on disk: it now fetches every branch
of each remote before pulling your own. \texttt{checkout} itself still
never touches the network, and still works offline --- it reads what the
last \texttt{pull} brought.

\begin{center}
\emph{To join somebody else's branch: \texttt{pull}, then \texttt{checkout
<their branch>}.}
\end{center}
...
\begin{itemize}
  ...
  \item \texttt{show} --- one State, every entry naming it, and what each
        \texttt{commit} wrote: the message, the repository, and whether the
        commit has been published. The argument is the State's content hash
        or any unambiguous prefix of it; nobody retypes sixty-four
        characters. \texttt{--full} prints whole messages instead of first
        lines, and the remote and ref each publication went to.
\end{itemize}
...
\texttt{status}, \texttt{list} and \texttt{show} are read-only; nothing
they do sends a memory anywhere.

\paragraph{Keeping a memory when the disk does
not.}\label{sec:memory-mount} A memory can be a
repository of its own, mounted at \texttt{.cgitsync} exactly as
\texttt{.localSpec} is: one repository holding every project's memory, on a
branch per project. Getting there is five commands, run once per project;
\texttt{tutorials/05\_memory.md} walks all five in order.
...
\begin{itemize}
  ...
  \item \texttt{memory adopt} --- makes the \texttt{.cgitsync} you already
        have into that repository, on this project branch's memory branch.
        Nothing on disk is moved, overwritten or lost, which is why this is
        a separate command from \texttt{clone}: \texttt{clone} refuses to
        run over a memory, and is right to.
...
Says which of four things is true of this workspace's recorded history.
...
\paragraph{The \texttt{SYNC} column.} It says how the branch a repository
...
\texttt{diverged(+N/-M)}   & Both, from a common ancestor. \texttt{merge}
                             or \texttt{pull-force} resolves it. \\
...
\paragraph{The memory mount reading \texttt{dirty}, or \texttt{HEAD} ending
in \texttt{*}, is normal.} If the tree mounts its own memory at
\texttt{.cgitsync} (\S\ref{sec:memory-mount}), two things about that one
row are expected, not a fault:

\begin{itemize}
  \item \textbf{\texttt{LOCAL} reads \texttt{dirty} after almost every
        command.} \texttt{add}, \texttt{commit}, and \texttt{push}
        deliberately never touch the memory's own working tree --- each of
        them still writes a record of itself \emph{into} the memory once
        it finishes acting on everything else, and that record is new,
        uncommitted content. \texttt{cgitsync memory push} is what sends
        it; running it leaves the row clean again until the next command
        writes its own record. When this row is the only thing dirty,
        \texttt{status} prints a note saying so and naming
        \texttt{memory push}.
...
Neither is worth chasing to a clean reading on purpose: a memory that
never showed anything pending would not be recording what happens.

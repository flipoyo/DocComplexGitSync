...
These are read-only. Nothing here sends a memory anywhere --- there is
nowhere to send it to yet.

\paragraph{Keeping a memory when the disk does not.} A memory can be a
...
\begin{lstlisting}[language=bash]
$ pixi run cgitsync merge <branch> [--private] [--ff-only|--no-ff] [--dry-run] [--resolve]
\end{lstlisting}

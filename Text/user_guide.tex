...
On a tree with private mounts, \texttt{br=} is where you see privacy at
work: the project's own repositories carry the branch you moved the tree to,
and each private mount still shows its own.
...
on a repository that is not private is rejected at validation: such a
repository is this project's own and is always writable.
...
\paragraph{Which commands take \texttt{--private}.} Eleven of them:
\texttt{pull}, \texttt{pull-force}, \texttt{checkout}, \texttt{branch},
\texttt{add}, \texttt{rm}, \texttt{commit}, \texttt{merge},
\texttt{push}, \texttt{tag} and \texttt{freeze}. The commands that act on
the tree as a whole --- \texttt{clone}, \texttt{initialise},
\texttt{freeze-release} and \texttt{launch-release} --- do not take it. It narrows the command to the writable
...
\paragraph{Privacy covers everything inside it.} A private repository

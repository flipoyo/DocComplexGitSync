...
\texttt{verify}.  The configuration level, \texttt{discover},
\texttt{configure} and
...
On a tree with pinned mounts, \texttt{br=} is where you see pinning at work:
the unpinned repositories carry the branch you moved the tree to, and each
pinned mount still shows its own.
...
\hline
\texttt{project}         & this project's own; write freely       & no \texttt{pinned} \\
\texttt{private/local}   & shared, and yours to write             & \texttt{pinned, writable} \\
\texttt{private/distant} & shared, read-only                      & \texttt{pinned} alone \\
\end{tabular}
\end{center}
...
\begin{lstlisting}[language=bash]
$ pixi run cgitsync merge <branch> [--private] [--ff-only|--no-ff] [--dry-run]
\end{lstlisting}
...
\texttt{repos[].pinned}           & one repository entry & \texttt{false} \\
...
\subsubsection{What \texttt{pinned} means}

A repository is \texttt{pinned} when it is \textbf{shared with other
projects} and must stay on its own branch.

\begin{center}
\emph{Branch moves stop at a pinned repository. Tags do not.}
\end{center}

\texttt{cgitsync branch <name>} and \texttt{cgitsync checkout <name>} skip
a pinned repository and leave it on its declared
...
A pinned repository is a \textbf{configuration repository}: shared with
other projects rather than owned by this one. \texttt{pinned = true} alone
...
\begin{lstlisting}
{ repository = "github:you/.sharedSpec", default_branch = "main", pinned = true },
{ repository = "github:you/.myNotes", default_branch = "MyProject", pinned = true, writable = true },
\end{lstlisting}
...
on an unpinned repository is rejected at validation: an unpinned repository
...
\begin{center}
\texttt{<the branch the entry declares>\_<the branch the project is on>}
\end{center}

With the project on \texttt{multi-branch}, an entry declaring
\texttt{default\_branch = "ComplexGitSync"} is checked out on
\texttt{ComplexGitSync\_multi-branch}. The separator is an underscore
because hyphens already occur inside branch names.
...
The derived branch is a \textbf{target, not a demand}: until it exists,
resolution falls back through the entry's own chain, so an existing tree
behaves exactly as it did.

\texttt{cgitsync branch <name>} is the only command that creates it.
\texttt{cgitsync checkout} deliberately does not: moving a tree must never
quietly create a branch in a repository shared with other projects.
...
A nested entry may restrict itself further --- writing \texttt{pinned =
...
then checked against the branch it is \emph{pinned} to, not against the
...
drafts a dot-named repository with \texttt{pinned = true} for that reason.
The rule is \emph{shared}, not \emph{hidden}, though: a plainly named
repository that two projects mount should be pinned too.

\textbf{The branch a repository is pinned to decides who sees your commits.}
A mount pinned to a branch named after your own project is yours to push
freely. A mount pinned to \texttt{main} is read by every project that mounts

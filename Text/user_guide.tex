...
\begin{itemize}
  ...
  \item \texttt{show} --- one State, every entry naming it, and what each
        ...
        characters. Pasted back verbatim from wherever it was printed also
        works: the bare prefix, the \texttt{state=<hash>} label
        \texttt{explore --timeline} prints it under, or the full
        \texttt{state(<hash>)} form a ledger entry uses. \texttt{--full}
        prints whole messages instead of first lines, and the remote and
        ref each publication went to.

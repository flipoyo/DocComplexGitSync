...
\paragraph{Every one of these that reaches a remote folds the memory first.}
When this tree mounts and has adopted a memory (\texttt{.cgitsync/.memory}),
\texttt{push()}, \texttt{tag()}, and \texttt{freeze()} each call
\texttt{memory\_push} on it before doing their own work --- regardless of
\texttt{private}, since the memory is this project's own record of itself,
not a configuration repository scoped by that flag. \texttt{client.last\_memory\_fold} holds what the most recent one of these folded: the
\texttt{memory\_push} result dict, or \texttt{None} when no memory is
mounted or the fold could not proceed. A fold that fails --- the memory
declared but not yet adopted, or unreachable for any other reason --- is
never raised: it is turned into a \texttt{warnings.warn(...)} call instead
(catchable with \texttt{pytest.warns}/\texttt{warnings.catch\_warnings}, the
same as \texttt{checkout}'s own build-change warning), so a Python caller
sees it exactly as a CLI user would, and the write it was folding for still
completes. \texttt{client.memory\_declared()} answers whether a memory is
mounted at all, with no side effect, for a caller that wants to know
in advance.

\paragraph{\texttt{close\_branch(branch\_name, private=False)}.} Renames
\texttt{branch\_name} to \texttt{closed/branch\_name} across the full
tree, leaf-first --- locally and on the remote, never deleting anything.
Requires \texttt{READY}. Raises \texttt{GitSyncError} before touching any
repository when \texttt{branch\_name} is the project's own default branch,
or when any repository in scope is currently checked out on it --- the
error names which repository. \texttt{client.last\_write\_outcomes} holds
one \texttt{RepoOutcome} per repository visited afterward: \texttt{acted}
is \texttt{False} for a repository that had no branch of that name at all,
never an error. \texttt{private=True} selects the writable configuration
repositories instead of the project's own, the same as \texttt{branch()}
(create).

\begin{lstlisting}[language=Python]
client.close_branch("memory-dev")
for outcome in client.last_write_outcomes:
    print(outcome.name, outcome.acted, outcome.detail)
\end{lstlisting}
...
# Merging into a branch that is not checked out, in one call.
client.merge_into_plan("multi-branch", "main")   # what it would do, per repo
client.merge_into("multi-branch", "main")        # checkout and merge, together
client.build_installed_from("main")              # the version that branch carries
...
\texttt{memory\_list}, \texttt{memory\_show}, \texttt{memory\_explore}}

The four read-only questions the CLI's \texttt{memory} group asks, each
...
status["verification"]  # verified / no-history / legacy / corrupt / time-inconsistent
...
state["tree"]                            # this State's own topology, like view_tree()
state["environments"]                    # [{"id": "env(<hash>)", "path": ..., "record": ...}]
for entry in state["entries"]:
    for commit in entry["commits"]:
        commit["message"], commit["repository"], commit["published"]
state["published"]                       # remote and ref each commit went to

environment = client.memory_show_environment("/path/to/CGSHOME", "env(<hash>)")
environment["record"]["machine"], environment["record"]["tools"]

answer = client.memory_explore("/path/to/CGSHOME")
answer["branch"]                         # the memory branch checked out here, or None
for commit in answer["commits"]:         # published commits, newest push first
    commit["repository"], commit["branch"], commit["sha"], commit["message"]

timeline = client.memory_explore("/path/to/CGSHOME", timeline=True)
for entry in timeline["entries"]:        # every ledger entry, in order
    entry["seq"], entry["command"], entry["state"], entry["commits"], entry["published"]
    # entry["state"] is what memory_show(cgshome, entry["state"]) takes
\end{lstlisting}

Commit messages come back whole. Shortening a long one to a single line is
the caller's decision, which is why the CLI has \texttt{--full} and this
method does not: a caller reading the memory from Python wants the message
that was written, not the one that fitted.

\texttt{memory\_show} raises \texttt{GitSyncError} when the prefix matches
no State, or more than one.

\texttt{memory\_explore}'s default view answers what a colleague pulling
this branch would see --- commits with no publication row are left out,
because that is \texttt{memory\_show}'s question, not this one.
\texttt{branch=} asks for a memory branch other than the one checked out on
this disk; today only the checked-out one can be read this way, and a
mismatch raises \texttt{GitSyncError} naming \texttt{memory\_clone(branch=...)}
rather than answering for the wrong branch.

The commands that give a memory a repository of its own have the same
mirror. \texttt{add\_memory\_repo\_cgs} is the one the owner named; the CLI
spells it \texttt{memory mount}.

\begin{lstlisting}[language=python]
client.repo_create("github:you/.memory", private=True)
#   {"created": "created" | "exists" | "unavailable", "command": "gh repo create ..."}

client.add_memory_repo_cgs("project.cgs")        # one entry, comments kept
client.memory_adopt("/path/to/CGSHOME")          # .cgitsync/.memory, fresh and empty
client.memory_push("/path/to/CGSHOME")           # folds .cgitsync's pending content in first
client.memory_branch("/path/to/CGSHOME", "main") # the branch a merge will need

# Only for a memory mounted before this command existed, directly at
# .cgitsync instead of .cgitsync/.memory:
client.memory_migrate("/path/to/CGSHOME", "project.cgs")
\end{lstlisting}

\texttt{repo\_create} never raises for a repository that already exists ---
that is \texttt{"exists"}, an ordinary answer --- and never for a provider
tool that is absent or signed out, which is \texttt{"unavailable"} carrying
the command to run. It raises only for a provider no tool is known for, and
for a tool that failed for some other reason. \texttt{memory\_list} includes a State that
...
\subsection{Starting a memory's history over: \texttt{memory\_reboot}}

\begin{lstlisting}[language=python]
answer = client.memory_reboot("/path/to/CGSHOME")
answer["folded"]         # pending records folded in before archiving
answer["archived_from"]  # the branch's name before the reboot
answer["archived_to"]    # "<branch>.archived-<YYYYMMDD>", real, on origin
answer["exported"]       # .cgitsync/.memory/.cgs/<project>-v<N>.cgs
answer["branch"]         # the original name, fresh: .cgs/ kept, else
                          # cleared, one State committed and pushed

# A fresh start from the very first adopt, instead of inheriting whatever
# the fallback branch already holds:
client.memory_adopt("/path/to/CGSHOME", remote=remote, branch=branch, reboot=True)
\end{lstlisting}

Nothing is ever force-pushed or deleted: the old branch is pushed to
origin under its archived name \emph{before} the old name is removed from
origin, so the commits it holds are reachable under some name on the
remote for the whole operation, never only locally. The fresh branch's
own genesis commit is pushed too, immediately, rather than left for the
next \texttt{memory\_push} --- the old name is already gone from origin by
then, so a bootstrap on another machine in between would otherwise find
no branch under this project's name at all. \texttt{memory\_reboot}
raises \texttt{GitSyncError} when the mount is not a repository yet, and
when today's archived name already exists --- a second reboot the same day
needs a name of its own rather than colliding with the first.

\subsection{Recording one piece of agent work: \texttt{self\_history\_add}}

\begin{lstlisting}[language=python]
from ComplexGitSync.memory.self_history import AgentInfo, ConformityCriterion, ConformityScore
...
conformity = ConformityScore(
    spec_respect=ConformityCriterion(score=33, basis="measured", reasoning="lint and test both pass"),
...
path = client.self_history_add(
    ...
    action="Wrote memory/self_history.py and self_history_add.",
    worker=worker,
...
Writes one record to \texttt{.cgitsync/.self-history/<hash>.toml}, content-addressed
the same way a State or an Environment record is. \texttt{contract} (the
current signed \texttt{AgentContractRecord}'s own hash) and
\texttt{checks.status\_errors} (this workspace's \texttt{errors=} count) are
filled in from what the client can itself observe; every other field,
including \texttt{lint\_passed}/\texttt{tests\_passed}, is the caller's own
account --- \texttt{self\_history\_add} cannot run \texttt{pixi run lint} or
\texttt{pixi run test} itself, since \texttt{subprocess} stays confined to
\texttt{git\_runner.py}, and that module runs Git, not Pixi.

\texttt{AgentInfo.role} is validated against \texttt{.localSpec/AGENT.md}'s
six-role roster; \texttt{ConformityCriterion.basis} against
\texttt{"measured"}/\texttt{"asserted"} only. Both raise
\texttt{ValueError} on anything else, the same way \texttt{goal}/
\texttt{action} raise for more than three lines.

\texttt{state\_before}/\texttt{state\_after} are \textbf{verified against
the ledger}, not merely shape-checked --- deliberately a ledger question,
not a \texttt{.cgs}/discovery one: a \texttt{.gts} is a static snapshot, so
whether a citation names something real is answered the same way
\texttt{verify} already answers it, via \texttt{MISSING\_STATE}/
\texttt{STATE\_DIGEST\_MISMATCH} — never by re-parsing or re-deriving
something from a transformed document. Concretely, each citation, if
given, must name a State some ledger entry actually recorded, on disk,
still hashing to its own name; one that fails any of the three raises
\texttt{GitSyncError} rather than being recorded as though it were fact.
\texttt{state\_after} is additionally \textbf{observed}, not merely
validated, when omitted: the ledger's own most recent entry at the moment
\texttt{self\_history\_add} runs.

\texttt{memory\_push} folds this pending directory into
\texttt{.cgitsync/.memory/.self-history/} and pushes it --- \emph{before}
folding and pushing \texttt{.memory} itself, once self-history has been
adopted (below). A tree that never adopted it sees no change: the fold is
a no-op the moment \texttt{.self-history}'s own mount has no \texttt{.git}.

\subsection{Adopting self-history: \texttt{self\_history\_adopt}}

\begin{lstlisting}[language=python]
result = client.self_history_adopt("/path/to/CGSHOME", owner="you")
result["mount"]   # .cgitsync/.memory/.self-history
result["branch"]  # .memory's own branch, unless a different one was given
result["remote"]  # the SSH URL self-history was adopted from
\end{lstlisting}

\texttt{self\_history\_adopt} is the \emph{one place} this fact is ever
decided: it writes \texttt{config-memory.cgs} the first time, for a
brand-new project or as the explicit retrofit for a \texttt{.memory}
adopted \emph{before} self-history existed (this project's own, among
others). Every precondition here raises \texttt{GitSyncError} instead of
returning quietly --- \texttt{.memory} not yet adopted, self-history
already adopted, no owner resolvable, or the remote not reachable.

There is no separate opt-in flag beyond that one decision:
\texttt{memory\_adopt} follows it automatically and silently afterward,
whenever \texttt{.memory}'s own already-fetched content already carries
\texttt{config-memory.cgs} --- typically because a project's own memory
branch inherited it from a fallback base branch that already had it, the
same mechanism that already lets a new project's memory share history
with another. This is deliberately \emph{not} a check against
\texttt{self.registry}: a \texttt{.gts} is a static snapshot of an
already-discovered tree, and \texttt{nested\_config} (the \texttt{.cgs}-side
declaration that makes \texttt{.self-history} a discoverable child of
\texttt{.memory}) is \texttt{.cgs}-only information a
\texttt{.gts}-loaded registry never carries in the first place --- asking
the registry would be the wrong layer, not a gap a runtime probe should
paper over. Reachability is checked too, but only \emph{after} that
question, to decide whether an already-declared repository can actually be
fetched right now.

\texttt{memory\_clone} brings self-history back too, reading which
repository to clone from \texttt{config-memory.cgs} itself rather than
re-deriving it, the moment that file says self-history was adopted
upstream. \texttt{memory\_reboot} leaves \texttt{.self-history} entirely
alone by construction: it only ever touches \texttt{.memory}'s own mount.

\subsection{Reading self-history: \texttt{memory\_self\_history}}

\begin{lstlisting}[language=python]
records = client.memory_self_history("/path/to/CGSHOME")
records[0]["ticket"]        # the short ticket name
records[0]["conformity"]    # {"spec_respect": {...}, "gating": {...}, "quality": {...}}
\end{lstlisting}

Every record this workspace holds, folded and pending, oldest first, each
as the same dict \texttt{SelfHistoryRecord.to\_dict()} produces --
consultation only. An empty list means no self-history has been written
...
\section{Environment observation and drift}

The client exposes the same read-only Environment operations as the CLI:
...
observed = client.environment()
print(observed.architecture, observed.pixi_platform, observed.digest())

requirements = CgsDocument.from_toml("/path/to/project.cgs")
drift = client.check_environment(requirements)
if not drift.matches:
    print("missing", drift.missing)
    print("older", drift.older)
print("observed but undeclared", drift.undeclared)
\end{lstlisting}

\texttt{environment()} returns a secret-free, content-addressable
\texttt{TreeEnvironment}. Credentials are booleans only; manifests are
tree-relative paths and digests, never copied contents. When a source
\texttt{.cgs} is available, \texttt{check\_environment()} may be called
without an argument. It returns data and installs nothing.
...
Calling \texttt{memory\_push} by hand, as above, is for settling the memory
on its own. \emph{READY Methods in the Client API}, earlier in this
chapter, covers the more common case: \texttt{push}, \texttt{tag}, and
\texttt{freeze} each call it automatically, first, once a memory is mounted
and adopted.
...
\section{Diagnosing and Repairing a Git Situation: \texttt{autofix}}
\label{sec:api-autofix}

\texttt{autofix(*, error=None, repo\_name=None)} reads the error the
tree's last command actually failed with, and runs whichever registered
repair recognises it --- the same command \texttt{cgitsync autofix}
wraps (\S\ref{sec:autofix} of the user guide):

\begin{lstlisting}[language=python]
from ComplexGitSync.orchestre import ComplexGitSyncClient

client = ComplexGitSyncClient()
client.load("/path/to/CGSHOME/snapshot.gts")

# error=None reads .cgitsync/logs/*.log's most recent failing command
outcome = client.autofix()

# Or hand it an error directly -- useful from a script that already
# caught one, or in a test
outcome = client.autofix(
    error="fatal: Not possible to fast-forward, aborting.",
    repo_name=".memory",
)

print(outcome.repaired, outcome.detail)
\end{lstlisting}

Returns an \texttt{autofix.RepairOutcome}: \texttt{repaired} (bool) and
\texttt{detail} (str, one line naming what changed or why nothing did).
\texttt{repaired=False} is not a failure --- it also covers the case
where the situation turned out not to need this repair at all (e.g.\ a
merge that applied cleanly on its own).

Raises \texttt{autofix.NoMatchingRepairError} (mapped to the CLI's
\texttt{EXIT\_REFUSED}, never a traceback, when reached through
\texttt{cgitsync autofix}) when no failing command is found to read, or
when nothing registered recognises the error --- \texttt{autofix} refuses
rather than guessing. It can also raise the ordinary
\texttt{errors.GitSyncError} a repair itself raises, for example when the
two sides of a divergence are not disjoint enough to reconcile safely
without a person's judgement.

\texttt{autofix} is deliberately not one growing class: each situation it
recognises lives in its own \texttt{ComplexGitSync.autofix.repair\_*}
module (today, \texttt{repair\_divergent\_user} --- a private repository
whose branch diverged because two machines wrote to it independently,
where the content itself, like \texttt{.memory}'s hash-chained ledger,
carries an ordering invariant a plain \texttt{merge} cannot see). Adding
a new situation this method can handle means adding a new module and
registering it, never editing an existing one to grow another branch.

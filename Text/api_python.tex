...
There is no separate opt-in flag: \texttt{memory\_adopt} already calls
this automatically, silently, the moment
\texttt{github:<owner>/.self-history} is reachable --- a project that has
never created that repository sees no new behaviour at all, since a
\texttt{.gts}-loaded registry never carries a nested-config declaration to
check in the first place. \texttt{self\_history\_adopt} is the explicit,
standalone form: the retrofit for a \texttt{.memory} that was adopted
\emph{before} self-history existed (this project's own, among others).
Unlike the automatic attempt, every precondition here raises
\texttt{GitSyncError} instead of returning quietly --- \texttt{.memory}
not yet adopted, self-history already adopted, no owner resolvable, or the

...
% narrative tutorials under tutorials/: this section gives the exact
...
\cgspkg{} ships a same-numbered tutorial under \texttt{tutorials/} for
each level below (\texttt{tutorials/README.md} lists them in order). Run
through them in order: each one adds exactly one new idea on top of the
last, so together they form a guided path from the simplest possible
\texttt{.cgs} to a real project with no \texttt{.cgs} of its own at all.

Levels~2 and~3 have their \texttt{.cgs} checked in under
\texttt{examples/}. Level~1 does not: \texttt{CGSil1.cgs} lives in the
\texttt{CGSil1} repository itself, which is what the transcript below
clones.
...
\texttt{tutorials/01\_first\_multi\_repo\_workspace.md} carries the
...
\texttt{tutorials/02\_onboarding\_a\_real\_build\_tree.md}.
...
children that were, until recently, plain git submodules. One of those
children, \texttt{HydrologicalTwinAlphaSeries}, holds a submodule of its
own at \texttt{docs/hydrological\_twin}: four repositories on two levels,
which is why every command below uses \texttt{--recursive}. It has no
...
independent routes, in \texttt{tutorials/03\_adopting\_a\_real\_project.md}.
...
# discover has something to find at those paths. --recursive reaches the
# second level too: without it, hydrological_twin stays an empty
# directory and discover cannot see a repository that is not there.
$ git clone https://gitlab.com/cawaqs/gviz/cawaqsviz cawaqsviz
$ cd cawaqsviz && git submodule update --init --recursive
...
\texttt{relative\_path = "."} and every other repository at its real
submodule path (\texttt{external/HydrologicalTwinAlphaSeries},
\texttt{docs/CWV\_user\_guide},
\texttt{external/HydrologicalTwinAlphaSeries/docs/hydrological\_twin})
rather than their bare repo names. That last path lies inside
\texttt{HydrologicalTwinAlphaSeries}, so the report marks it
\texttt{inside: external/HydrologicalTwinAlphaSeries} and the file is read
back with \texttt{HydrologicalTwinAlphaSeries} as its parent. No repository
here has a \texttt{.cgs} of its own, and \texttt{discover} writes no
\texttt{nested\_config} override for that: the default \texttt{"auto"}
already resolves a repository with zero nested \texttt{*.cgs} matches
cleanly. See \texttt{discover} in \S\ref{chap:user-guide} for exactly
...
\texttt{cawaqsviz} tracked its children as real git submodules. This mode
...
# reports name, path, URL, branch and declaring .gitmodules per submodule;
# changes nothing. --recursive reaches HydrologicalTwinAlphaSeries' own
# submodule; without it, only the root's two are converted.
$ pixi run cgitsync import-submodules /path/to/cawaqsviz --recursive

# Convert: drop the gitlinks, retire .gitmodules, extend .gitignore.
$ pixi run cgitsync import-submodules /path/to/cawaqsviz --recursive --apply

# Review and commit the resulting working-tree changes as usual. Two
# repositories have changes here, one per converted level.
...
\texttt{tutorials/03\_adopting\_a\_real\_project.md} \S4 --- this

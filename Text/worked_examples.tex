...
\cgspkg{} ships four checked-in \texttt{examples/} topologies. Levels~1
and~2 each have one matching tutorial under \texttt{docs/tutorials/};
Level~3's three routes span two more, since the third route
(\texttt{import-submodules}) also covers reaching a \texttt{READY} tree
(\texttt{docs/tutorials/README.md} lists all four tutorials in order). Run
through the levels in order: each one adds exactly one new idea on top of
the last, so together they form a guided path from the simplest possible
...
Full narrative in
\texttt{docs/tutorials/03\_configuration\_discovery\_modes.md} (routes 3a--3b)
and \texttt{docs/tutorials/04\_submodules\_to\_ready.md} (route 3c).
...
repositories are in \texttt{docs/tutorials/04\_submodules\_to\_ready.md}
\S1 --- this level is the condensed command sequence, not a restatement of
that walkthrough. \texttt{import-submodules} and \texttt{discover} answer

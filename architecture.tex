...
\hline
\texttt{load\_cgs}               & any         & DECLARED \\
\texttt{load\_gts}               & any         & READY    \\
\texttt{discover\_nested\_configs} & DECLARED    & DECLARED (enriched) \\
\texttt{clone\_cgs}              & any         & \textbf{READY} \\
\texttt{restart}                 & DECLARED    & \textbf{READY} \\
\texttt{checkout}                & READY       & \textbf{READY} \\
\texttt{commit}                  & \textbf{READY only} & READY \\
\texttt{push}                    & \textbf{READY only} & READY \\
\texttt{tag}                     & \textbf{READY only} & READY \\
\texttt{freeze\_release}         & \textbf{READY only} & READY \\
\texttt{launch\_release}         & any (from \texttt{.gts}) & READY \\
\hline
\end{tabular}
\end{center}
...
\caption{Synchronisation operations: \texttt{checkout\_tree} runs
  propagate\,→\,create\,→\,git checkout parent-first; \texttt{commit\_tree}
  and \texttt{push\_tree} iterate leaf-first so that inner repos commit and
  push before their parents.}
...
\caption{Client state-gating: the Client reads \texttt{TreeLifecycleState}
  ...
  \texttt{commit}, \texttt{push}, \texttt{tag}, \texttt{freeze\_release}) are
  blocked unless the tree is \texttt{READY}.}

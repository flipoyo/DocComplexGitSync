...
\caption{Client state-gating: the Client reads \texttt{TreeLifecycleState}
  before delegating to an action.  Mutation actions (\texttt{commit},
  \texttt{push}, \texttt{tag}, \texttt{freeze\_release}) are blocked unless the
  tree is \texttt{READY}.}
...
src/ComplexGitSync/
  ...
  operations.py                (planned: checkout, commit, push, tag, …)

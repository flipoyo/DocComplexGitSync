...
Resynchronizes an already-cloned project tree. Loads the \texttt{.cgs}
configuration, discovers nested configs, reads the root repository's current
branch, and checks out that branch across the full tree parent-first.
Ends in \texttt{READY} or fails explicitly. Also writes a \texttt{.gts}
snapshot on success.
...
resolved branch.  Requires a \texttt{READY} tree; the tree remains
...
READY registry from \texttt{--gts}.  Requires a \texttt{READY} tree and keeps
the tree \texttt{READY} on success.
...
\begin{lstlisting}[language=bash]
$ cgitsync freeze-release <name> --gts <snapshot.gts>
\end{lstlisting}
...
\textbf{GitTreeState} snapshot, and \texttt{.goc} means
\textbf{GitOrchestrationCommand} plan.
...
\texttt{READY} on success. \texttt{launch} loads a release
\texttt{.gts}, performs required clone/checkout actions, and must end in
\texttt{READY}. \texttt{checkout} and \texttt{freeze} also write new
\texttt{.gts} snapshots.
